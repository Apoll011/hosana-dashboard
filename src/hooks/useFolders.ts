...
export function useFolders() {
  ...
  const renameFolderMutation = useMutation({
    mutationFn: ({ id, name, updatedAt }: { id: string; name: string; updatedAt: string }) =>
  ...
  const moveFolderMutation = useMutation({
    mutationFn: ({ id, parentId, updatedAt }: { id: string; parentId: string | null; updatedAt: string }) =>
  ...
  const deleteFolderMutation = useMutation({
    mutationFn: ({ id, action }: { id: string; action: 'delete_songs' | 'move_to_root' }) =>
