...
import { Tag, Plus, X, Check, Tag as TagIcon, Sparkles } from 'lucide-react';
...
interface BatchTagModalProps {
  ...
  onConfirm: (tags: string[], mode: 'append' | 'replace' | 'remove') => Promise<void>;
}
    ...
    >
      <div className="flex flex-col gap-5 text-slate-700 dark:text-slate-300">
        <p className="text-xs text-slate-500 dark:text-slate-400">
          ...
          <strong className="text-slate-900 dark:text-slate-100">{count} cântico(s) selecionado(s)</strong>.
        </p>
        ...
        <div className="flex flex-col gap-2">
          ...
          <span className="text-[11px] text-slate-500 dark:text-slate-400 italic">
            {mode === 'append' && '• Acrescenta estas etiquetas mantendo as existentes nos cânticos.'}
            {mode === 'replace' && '• Substitui todas as etiquetas existentes pelas etiquetas selecionadas abaixo.'}
            {mode === 'remove' && '• Remove estas etiquetas específicas de todos os cânticos selecionados.'}
          </span>
        </div>
        ...
        {tags.length > 0 && (
          <div className="flex flex-col gap-1.5 p-3 bg-sky-50/70 dark:bg-sky-950/40 border border-sky-200 dark:border-sky-800/60 rounded-xl">
            <span className="text-[10px] font-bold uppercase tracking-wider text-[#0284c7]">
              Etiquetas a {mode === 'remove' ? 'remover' : 'aplicar'} ({tags.length})
            </span>
