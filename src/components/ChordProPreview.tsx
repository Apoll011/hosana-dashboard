              ...
              <div>
                ...
                {parsedSong.artist && (
                  <div className="flex items-center gap-1 text-xs text-neutral-500 mt-2 font-medium">
                    ...
                    <span>Por: {parsedSong.artist}</span>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-1.5 justify-end">
                ...
                {parsedSong.key && (
                  <span className="text-[10px] font-bold bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 px-2.5 py-1 rounded-lg border border-indigo-100 dark:border-indigo-950/50">
                    Tom Original: {parsedSong.key}
                  </span>
                )}
                {parsedSong.capo && parsedSong.capo !== '0' && (
                ...
                return (
                  <div key={secIdx} className="bg-slate-100 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700 my-4 select-text">
                    ...
                    <pre className="font-mono text-xs text-slate-800 dark:text-slate-200 overflow-x-auto leading-relaxed whitespace-pre">
                      {section.lines.map(line => line.rawText || '').join('\n')}
                    </pre>
                  </div>
                );
              }
              ...
              if (section.type === 'comment') {
                return (
                  <div key={secIdx} className="italic text-xs text-slate-500 bg-slate-100 dark:bg-slate-800/50 px-4 py-2 rounded-xl border-l-2 border-slate-200 dark:border-slate-700 my-2 select-none">
                    {section.lines.map(l => l.rawText).join(', ')}
                  </div>
                );
              }
  ...
  if (line.type === 'comment') {
    return (
      <div className="text-xs text-slate-400 dark:text-slate-500 italic">
        {line.directiveValue || line.rawText}
      </div>
    );
  }
